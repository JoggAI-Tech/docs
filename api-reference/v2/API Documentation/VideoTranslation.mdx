...
|--------|----------|-------------|
| GET | `/open/v2/video_translate/target_languages` | Get supported languages |
| POST | `/open/v2/video_translate/` | Submit translation task |
| GET | `/open/v2/video_translate/{translate_id}` | Check translation status |
...
```bash
curl --request POST \
  --url 'https://api.jogg.ai/open/v2/video_translate/' \
  --header 'x-api-key: YOUR_API_KEY' \
...
```bash
curl --request GET \
  --url 'https://api.jogg.ai/open/v2/video_translate/trans_abc123' \
  --header 'x-api-key: YOUR_API_KEY'
```
