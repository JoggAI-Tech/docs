...
| `GET /template_video/{id}` | Check video status | [API Reference](/api-reference/v2/Video/CreateVideoWithTemplate) |
...
```bash
curl --request GET \
  --url 'https://api.jogg.ai/open/v2/templates/custom?aspect_ratio=0' \
...
<Check>
Filter by `aspect_ratio`: -1=All, 0=Portrait, 1=Landscape, 2=Square
</Check>
...
```bash
curl --request GET \
  --url 'https://api.jogg.ai/open/v2/template/custom/1234' \
  --header 'x-api-key: YOUR_API_KEY'
```
...
```bash
curl --request POST \
  --url 'https://api.jogg.ai/open/v2/create_video_with_template' \
  --header 'x-api-key: YOUR_API_KEY' \
...
```bash
curl --request GET \
  --url 'https://api.jogg.ai/open/v2/template_video/video_123456' \
  --header 'x-api-key: YOUR_API_KEY'
```
...
2. Use [Webhooks](/api-reference/v2/API Documentation/WebhookIntegration) for notifications
