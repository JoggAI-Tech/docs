---
title: "Install MCP Clients"
description: "Configure Jogg Remote MCP in Codex, Claude Code, Cursor, or ChatGPT with one client-aware command."
---

JoggAI provides a small installer that applies the correct MCP and OAuth settings
...
<Warning>
Jogg Remote MCP is in controlled beta. The installer cannot enable the hosted
...
<Note>
For Cursor, open **Customize > MCP**, select Jogg, and complete OAuth after the
...
Installation is idempotent. If `jogg` already points to the production URL,
...
| Option | Purpose |
| --- | --- |
| `--name NAME` | Change the local MCP server name from `jogg` |
| `--url URL` | Target a non-production Jogg MCP deployment |
...
By default, the installer requests all 14 currently supported scopes so the
